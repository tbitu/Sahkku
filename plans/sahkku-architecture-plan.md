...
|---|---|---|---|---|---|
| `1-engine-decisions-and-state` | `plans/tasks/1-engine-decisions-and-state.md` | Add explicit reroll choice mechanics, throw-for-start integration, and state serialization to pure-C# `Sahkku.Rules`. | N/A | Decision completeness & LLM state formatting | draft |
| `2-match-controller-and-hotseat` | `null` | Refactor `GameLogic` into `MatchController` driving `IPlayerAgent` slots; implement 2P hotseat UX and `HumanPlayerAgent`. | `1-engine-decisions-and-state` | Compartmentalization & 2-player local play | not_created |
